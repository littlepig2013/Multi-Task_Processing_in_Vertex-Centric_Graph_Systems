...
CONF=" -f ~/vc-systems/GraphD/examples/basic_graphD_ppr/test_tmp_conf"
EXECUTABLE_FILE=" ~/vc-systems/GraphD/examples/basic_graphD_ppr/run"
...
def method3(W, M, params, params2):#min(f(w)+g(W-w))
	...
	while True:
		...
		min_mem = float("inf")
		for i in range(1, W+1):
			tmp_result = (func_params(i, params) + func2_params(W-i, params2)) - func_params(0, params)
			if tmp_result > 0 and tmp_result < min_mem:
				min_mem = tmp_result
				final_w = i
		if final_w == 0 or M < min_mem:
			break
		W -= final_w
...
if __name__ == '__main__':
	...
	for machineNum in machine_list:
		...
		os.system("head -" + str(machineNum) + " ~/PregelPlus/examples/basic_pregelplus_ppr/tmp_conf > test_tmp_conf")
