...
public:
	...
	void recv()
	{
		...
		StopTimer(MERGE_SORT_TIMER);
		string report1 = "--- --- Sort Merge Time ";
			PrintTimer(report1.c_str(), MERGE_SORT_TIMER);
			ResetTimer(MERGE_SORT_TIMER);		
		}
		else
		{//Case 2: len == 0, endTag
	...
	void run()
	{
		...
		while(true)
		{
			...
			rcount = 0;
			batNum = 0;
		ResetTimer(RECVER_XMACHINEs_TIMER);
			//------
			do{
		...
		StopTimer(RECVER_XMACHINEs_TIMER);
		string report0 = "--- --- Sync XMachines Time ";
			PrintTimer(report0.c_str(), RECVER_XMACHINEs_TIMER);
			ResetTimer(RECVER_XMACHINEs_TIMER);	
		ResetTimer(MERGE_SORT_TIMER);
		...
		StopTimer(RECVER_LOCK_TIMER);
		string report2 = "--- --- Recver Lock (including all_sum(active vertexes)) Time ";
			PrintTimer(report2.c_str(), RECVER_LOCK_TIMER);
			ResetTimer(RECVER_LOCK_TIMER);	
