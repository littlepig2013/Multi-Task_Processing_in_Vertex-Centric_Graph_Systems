...
/**
  ...
  */
public class JsonMSSPVertexInputFormat extends
  TextVertexInputFormat<LongWritable, MSSPVertexValue, FloatWritable> {
  ...
  */
  class JsonLongDoubleFloatDoubleVertexReader extends
    ...
    @Override
    protected MSSPVertexValue getValue(JSONArray jsonVertex) throws
      JSONException, IOException {
        int index =0;
        Long id = jsonVertex.getLong(index);
        index++;
        int size = jsonVertex.getInt(index);
        index++;
        Map<Long, Double> hashValues = new HashMap<Long, Double>();
        for(int i =0;i<size;i++){
          Long sourceId = jsonVertex.getLong(index);
          index++;
          Double value = jsonVertex.getDouble(index);
          index++;
          hashValues.put(sourceId, value);
        }
      return new MSSPVertexValue(hashValues);
    }

    @Override
    protected Iterable<Edge<LongWritable, FloatWritable>> getEdges(
        JSONArray jsonVertex) throws JSONException, IOException {
          int size = jsonVertex.getInt(1);
      JSONArray jsonEdgeArray = jsonVertex.getJSONArray(2+2*size);
    ...
    @Override
    protected Vertex<LongWritable, MSSPVertexValue, FloatWritable>
