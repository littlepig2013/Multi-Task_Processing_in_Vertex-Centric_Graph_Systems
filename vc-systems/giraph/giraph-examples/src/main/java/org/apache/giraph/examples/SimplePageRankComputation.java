...
public class SimplePageRankComputation extends BasicComputation<LongWritable,
  ...
  public static final int MAX_SUPERSTEPS = 57;
  /** Logger */
  private static final Logger LOG =
      Logger.getLogger(SimplePageRankComputation.class);
  private static float tol = 0.01f;
  private static final FloatConfOption tolOption =
            new FloatConfOption("SimplePageRankComputation.tol",
                   tol, "tolerence in pagerank");
  /** Sum aggregator name */
  ...
  public void compute(
    ...
    if (getSuperstep() >= 1) {
      ...
      DoubleWritable oldVertexValue = vertex.getValue();
      vertex.setValue(vertexValue);
      /*
      if((float)Math.abs(oldVertexValue.get() - vertexValue.get()) < tol){
          vertex.voteToHalt();
      }else{
      ...
      if (LOG.isInfoEnabled()) {
        LOG.info("next: Return vertexId=" + vertex.getId().get() +
            ", delta_vertexValue=" + Math.abs(oldVertexValue.get() - vertexValue.get()));
      }
