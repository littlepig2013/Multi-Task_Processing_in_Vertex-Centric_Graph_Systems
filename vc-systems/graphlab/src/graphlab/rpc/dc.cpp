...
distributed_control::distributed_control() {
  ...
  if (init_param_from_env(initparam)) {
    ...
    logstream(LOG_EMPH) << "Distributed Control Initialized from Environment" << std::endl;
  } else if (init_param_from_zookeeper(initparam)) {
      //logstream(LOG_INFO) << "Distributed Control Initialized from Zookeeper" << std::endl;
      logstream(LOG_EMPH) << "Distributed Control Initialized from Zookeeper" << std::endl;
      ...
      logstream(LOG_EMPH) << "Distributed Control Initialized from MPI" << std::endl;
  }
  else {
    //logstream(LOG_INFO) << "Shared Memory Execution" << std::endl;
    logstream(LOG_EMPH) << "Shared Memory Execution" << std::endl;
...
void distributed_control::init(const std::vector<std::string> &machines,
  ...
  logstream(LOG_EMPH) << "TCP Communication layer constructed." << std::endl;
  if (localprocid == 0) {
    ...
    logstream(LOG_EMPH) << "Address: " << address << std::endl;
      ipaddresses[address]++;
    }
