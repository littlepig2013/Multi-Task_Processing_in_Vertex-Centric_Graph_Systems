...
void pregel_send(void* buf, unsigned int size, int dst)
{
    ...
    MPI_Send(buf, int(tmp_size), MPI_SHORT, dst, 0, MPI_COMM_WORLD);
}

void pregel_recv(void* buf, unsigned int size, int src)
{
    ...
    if(int(tmp_size) < 0) cerr << size << endl;
    MPI_Recv(buf, int(tmp_size), MPI_SHORT, src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
